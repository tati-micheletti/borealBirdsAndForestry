...
library("raster")
areaAndAbundance <- function(filepath, type, tablePerPixel = NULL, rasterToMatch = NULL, onlyNA = FALSE){
  originalDensityRasters <- lapply(X = filepath, FUN = function(rasPath){
    ...
    birdRas <- Cache(reproducible::postProcess, birdRas, rasterToMatch = rasterToMatch, filename2 = paste0("lcc05Density", bird),
                     destinationPath = dirname(filepath), overwrite = TRUE,
                     userTags = c("objectName:birdRas", "script:paperPlots", "goal:reprojectedDensityBIRDtoLCC05"))
    birdDT <- data.table::data.table(species = bird, density = raster::getValues(birdRas), 
