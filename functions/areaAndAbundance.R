...
library("raster")
areaAndAbundance <- function(filepath, type){
  originalDensityRasters <- lapply(X = filepath, FUN = function(rasPath){
    ...
    dt[, c("totalAreaHa", ifelse(type == "density", "realAbund0", paste0("abund", type))) := list(NROW(birdDT)*areaHa, sum(birdDT$density*areaHa))]
