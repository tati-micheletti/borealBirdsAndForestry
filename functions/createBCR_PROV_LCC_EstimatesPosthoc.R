createBCR_PROV_LCC_EstimatesPosthoc <- function(BCR, pathToDSave,
  ...
  rasBCR[] <- rasBCR[]
  rasBCR <- Cache(postProcess, rasBCR, rasterToMatch = raster(densityMap), filename2 = file.path(pathToDSave, "RTMedBCR_posthoc"), format = "GTiff")
  ...
  rasPROV <- Cache(postProcess, rasPROV, rasterToMatch = raster(densityMap), filename2 = file.path(pathToDSave, "RTMedPROV_posthoc"), format = "GTiff")
