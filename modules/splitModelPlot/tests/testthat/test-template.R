...
#    `functionName` is a function's name in your module (e.g., `splitModelPlotEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ian/Tati/borealBirdsAndForestry/modules/splitModelPlot/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("splitModelPlot")
  ...
  parameters <- list(
    ...
    splitModelPlot = list(.saveInitialTime = NA)
  )
  ...
  if (exists("splitModelPlotEvent1", envir = .GlobalEnv)) {
    simOutput <- splitModelPlotEvent1(mySim)
  } else {
  ...
  if (exists("splitModelPlotEvent2", envir = .GlobalEnv)) {
    simOutput <- splitModelPlotEvent2(mySim)
  } else {
