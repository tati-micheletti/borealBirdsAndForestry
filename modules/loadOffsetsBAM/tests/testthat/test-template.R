...
#    `functionName` is a function's name in your module (e.g., `loadOffsetsBAMEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/borealBirdsAndForestry/modules/loadOffsetsBAM/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("loadOffsetsBAM")
  ...
  parameters <- list(
    ...
    loadOffsetsBAM = list(.saveInitialTime = NA)
  )
  ...
  if (exists("loadOffsetsBAMEvent1", envir = .GlobalEnv)) {
    simOutput <- loadOffsetsBAMEvent1(mySim)
  } else {
  ...
  if (exists("loadOffsetsBAMEvent2", envir = .GlobalEnv)) {
    simOutput <- loadOffsetsBAMEvent2(mySim)
  } else {
