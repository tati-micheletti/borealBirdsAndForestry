applyFocalToTiles <- function(#useParallel = P(sim)$useParallel, # Should do paralell only for focal and predicting, maybe?
  ...
  # } else {
    ...
    gc()
    # mergedFocalTiles <- SpaDES.tools::mergeRaster(focalTilesToMerge) # When mergeRaster is fixed for mosaic 
    # function (max instead of mean, passable as argument), can exchange 
