...
#    `functionName` is a function's name in your module (e.g., `focalCalculationEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/borealBirdsAndForestry/modules/focalCalculation/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("focalCalculation")
  ...
  parameters <- list(
    ...
    focalCalculation = list(.saveInitialTime = NA)
  )
  ...
  if (exists("focalCalculationEvent1", envir = .GlobalEnv)) {
    simOutput <- focalCalculationEvent1(mySim)
  } else {
  ...
  if (exists("focalCalculationEvent2", envir = .GlobalEnv)) {
    simOutput <- focalCalculationEvent2(mySim)
  } else {
