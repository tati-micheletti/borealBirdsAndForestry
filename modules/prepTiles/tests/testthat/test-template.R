...
#    `functionName` is a function's name in your module (e.g., `prepTilesEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/borealBirdsAndForestry/modules/prepTiles/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("prepTiles")
  ...
  parameters <- list(
    ...
    prepTiles = list(.saveInitialTime = NA)
  )
  ...
  if (exists("prepTilesEvent1", envir = .GlobalEnv)) {
    simOutput <- prepTilesEvent1(mySim)
  } else {
  ...
  if (exists("prepTilesEvent2", envir = .GlobalEnv)) {
    simOutput <- prepTilesEvent2(mySim)
  } else {
