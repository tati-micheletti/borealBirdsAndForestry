...
#    `functionName` is a function's name in your module (e.g., `birdDensityBCR_Prov_LCCEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/borealBirdsAndForestry/modules/birdDensityBCR_Prov_LCC/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("birdDensityBCR_Prov_LCC")
  path <- list(modulePath = "/home/tmichele/Documents/GitHub/borealBirdsAndForestry/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    birdDensityBCR_Prov_LCC = list(.saveInitialTime = NA)
  )
  ...
  if (exists("birdDensityBCR_Prov_LCCEvent1", envir = .GlobalEnv)) {
    simOutput <- birdDensityBCR_Prov_LCCEvent1(mySim)
  } else {
  ...
  if (exists("birdDensityBCR_Prov_LCCEvent2", envir = .GlobalEnv)) {
    simOutput <- birdDensityBCR_Prov_LCCEvent2(mySim)
  } else {
