...
#    `functionName` is a function's name in your module (e.g., `GISPrepEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ian/Tati/borealBirdsAndForestry/modules/GISPrep/tests/testthat`).
...
test_that("test Event1 and Event2.", {
  ...
  parameters <- list(
    ...
    GISPrep = list(.saveInitialTime = NA)
  )
  ...
  if (exists("GISPrepEvent1", envir = .GlobalEnv)) {
    simOutput <- GISPrepEvent1(mySim)
  } else {
  ...
  if (exists("GISPrepEvent2", envir = .GlobalEnv)) {
    simOutput <- GISPrepEvent2(mySim)
  } else {
