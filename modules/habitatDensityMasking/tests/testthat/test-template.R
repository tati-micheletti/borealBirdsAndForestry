...
#    `functionName` is a function's name in your module (e.g., `habitatDensityMaskingEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/borealBirdsAndForestry/modules/habitatDensityMasking/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("habitatDensityMasking")
  ...
  parameters <- list(
    ...
    habitatDensityMasking = list(.saveInitialTime = NA)
  )
  ...
  if (exists("habitatDensityMaskingEvent1", envir = .GlobalEnv)) {
    simOutput <- habitatDensityMaskingEvent1(mySim)
  } else {
  ...
  if (exists("habitatDensityMaskingEvent2", envir = .GlobalEnv)) {
    simOutput <- habitatDensityMaskingEvent2(mySim)
  } else {
