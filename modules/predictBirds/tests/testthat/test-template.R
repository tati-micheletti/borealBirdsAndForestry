...
#    `functionName` is a function's name in your module (e.g., `predictBirdsEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/tmichele/GitHub/borealBirdsAndForestry/modules/predictBirds/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("predictBirds")
  ...
  parameters <- list(
    ...
    predictBirds = list(.saveInitialTime = NA)
  )
  ...
  if (exists("predictBirdsEvent1", envir = .GlobalEnv)) {
    simOutput <- predictBirdsEvent1(mySim)
  } else {
  ...
  if (exists("predictBirdsEvent2", envir = .GlobalEnv)) {
    simOutput <- predictBirdsEvent2(mySim)
  } else {
