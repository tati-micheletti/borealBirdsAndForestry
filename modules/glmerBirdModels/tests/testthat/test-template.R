...
#    `functionName` is a function's name in your module (e.g., `glmerBirdModelsEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/borealBirdsAndForestry/modules/glmerBirdModels/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("glmerBirdModels")
  ...
  parameters <- list(
    ...
    glmerBirdModels = list(.saveInitialTime = NA)
  )
  ...
  if (exists("glmerBirdModelsEvent1", envir = .GlobalEnv)) {
    simOutput <- glmerBirdModelsEvent1(mySim)
  } else {
  ...
  if (exists("glmerBirdModelsEvent2", envir = .GlobalEnv)) {
    simOutput <- glmerBirdModelsEvent2(mySim)
  } else {
