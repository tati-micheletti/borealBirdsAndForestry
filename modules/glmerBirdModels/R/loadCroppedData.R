...
loadCroppedData <- function(data){
  ...
  if (params(sim)$glmerBirdModels$cropForModel==TRUE){
    ...
    if (is.null("birdData")){ # Still to test for automatic download of data. If it doesn't work: try exists(data.path) and uncommenting line above
