...
#    `functionName` is a function's name in your module (e.g., `finalRasterPlotsEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/borealBirdsAndForestry/modules/finalRasterPlots/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("finalRasterPlots")
  ...
  parameters <- list(
    ...
    finalRasterPlots = list(.saveInitialTime = NA)
  )
  ...
  if (exists("finalRasterPlotsEvent1", envir = .GlobalEnv)) {
    simOutput <- finalRasterPlotsEvent1(mySim)
  } else {
  ...
  if (exists("finalRasterPlotsEvent2", envir = .GlobalEnv)) {
    simOutput <- finalRasterPlotsEvent2(mySim)
  } else {
