...
#    `functionName` is a function's name in your module (e.g., `disturbanceDensityEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ian/Tati/borealBirdsAndForestry/modules/disturbanceDensity/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("disturbanceDensity")
  path <- list(modulePath = "C:/Ian/Tati/borealBirdsAndForestry/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    disturbanceDensity = list(.saveInitialTime = NA)
  )
  ...
  if (exists("disturbanceDensityEvent1", envir = .GlobalEnv)) {
    simOutput <- disturbanceDensityEvent1(mySim)
  } else {
  ...
  if (exists("disturbanceDensityEvent2", envir = .GlobalEnv)) {
    simOutput <- disturbanceDensityEvent2(mySim)
  } else {
