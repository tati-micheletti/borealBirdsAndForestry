...
#    `functionName` is a function's name in your module (e.g., `birdDensityTrendsEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/tmichele/GitHub/borealBirdsAndForestry/modules/birdDensityTrends/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("birdDensityTrends")
  path <- list(modulePath = "C:/Users/tmichele/GitHub/borealBirdsAndForestry/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    birdDensityTrends = list(.saveInitialTime = NA)
  )
  ...
  if (exists("birdDensityTrendsEvent1", envir = .GlobalEnv)) {
    simOutput <- birdDensityTrendsEvent1(mySim)
  } else {
  ...
  if (exists("birdDensityTrendsEvent2", envir = .GlobalEnv)) {
    simOutput <- birdDensityTrendsEvent2(mySim)
  } else {
