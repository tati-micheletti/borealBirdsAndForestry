buildYearlyDT <- function(currentTime,
  ...
  # FAILING. Not sure why, but might have to do with a bigger study area than Raster? 
  # birdDensityRasters = logDENSITY_BIRD
  # density <- log(reproducible::postProcess(x = birdDensityRasters, destinationPath = file.path(getwd(), "modules/bayesianBirdModel/data/"),
  #                        studyArea = rP, 
  #                        filename2 = NULL))
  density <- log(birdDensityRasters)
  ...
  invisible(lapply(X = focalNames, function(ras){
    ...
    #                   filename2 = NULL, rasterToMatch = density)
    fc <- focalRasters[[ras]]
  ...
  predTable <- lapply(X = c("density", "State_P_100", "State_P_500"), 
                      FUN = function(ras){
                        ras <- get(ras)                    
