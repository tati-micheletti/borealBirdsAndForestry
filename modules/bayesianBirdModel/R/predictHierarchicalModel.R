predictHierarchicalModel <- function(bird,
  ...
  #     # mu.beta[1:2] ~ dmnorm(mean.mu.beta[], tau.beta[,]) #  conjugate prior of the mean vector is another multivariate normal distribution
  #     mean.mu.beta ~ dunif(-40, 40)
  #     mu.beta[1:2] ~ dnorm(mean.mu.beta, 0.1)
  #     R[1:2, 1:2] ~ dwish(omega.beta[,], 2) # the conjugate prior of the covariance matrix is an inverse-Wishart distribution
  #     # sd.beta1 <- 1/pow(tau.beta[,1], -2) # cluster heterogeneity in lambda
  #     # sd.beta2 <- 1/pow(tau.beta[,2], -2) # cluster heterogeneity in lambda
  ...
  #                        tau.beta = matrix(c(10, 0, 0, 10), nrow = 2),
  ...
  #   )
  # 
  #   params <- c("beta", "mu.beta", #"tau.beta", "omega.beta", "sd.beta", 
  #               "lambda", "omega", "phi", 
  ...
  #                        switchCluster = 1, # or 0 to turn off random effects of Cluster
  #                        mean.mu.beta = c(0, 0),
  #                        tau.beta = matrix(c(100, 0, 0, 100), nrow = 2),
  #                        omega.beta = matrix(c(100, 0, 0, 100), nrow = 2)
  #   )
  #   
  ...
  #     R = matrix(c(1,0,0,1), nrow = 2)
  #   )
  #   params <- c("beta", "mu.beta", "R", #"tau.beta", #"sd.beta", 
  ...
  if (modelType == 1){
    ################################### MODEL 1: 1 equation no ZI
    message("Running model 1 for ", bird)
    iSMCode <- nimbleCode({
    ...
    iSMinits <- list(
      # Coefficients
      beta = runif(Nbeta, min = -40, 40), # Coefficient for disturbance ~ Has distribution, not sure need to provide
      tau.year = runif(1, 0.01, 10), # Deviation of year RE ~ Has distribution
      tau.clusters = runif(1, 0.01, 10), # Deviation of cluester RE ~ Has distribution
      tau.beta = runif(1, 0.01, 10), # Deviation of beta hyperparameter ~ Has distribution
    ...
    params <- c("mu.beta", "beta", "R", #"tau.beta", #"sd.beta", 
                "lambda", "mean.mu.beta",
                "clusterRanEff", "YearRanEff")
  }
  
  if (modelType == 2){
    ################################### MODEL 2: 2 equations no ZI
    message("Running model 1 for ", bird)
    iSMCode <- nimbleCode({
    ...
    iSMinits <- list(
      # Coefficients
      beta = runif(Nbeta, min = -40, 40), # Coefficient for disturbance ~ Has distribution, not sure need to provide
      logNN =  log(runif(nvisits, 0, 5)), # Distribution of lambda1 ~ Has distribution, not sure need to provide
      lambdaL = lambdaL, # Local model, poisson of counts <- Is assigned
      loglambdaL = log(lambdaL), # log of lambda2
      tau.year = runif(1, 0.01, 10), # Deviation of year RE ~ Has distribution
      tau.clusters = runif(1, 0.01, 10), # Deviation of cluester RE ~ Has distribution
