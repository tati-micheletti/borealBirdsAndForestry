runModel1 <- function(birdData, 
                       bird, startTime, currentTime){
  
  message("Running model 2 multivariate model for ", bird)
  iSMCode <- nimbleCode({
    ######## PRIORS ########
    ...
    sd.cluster <-
      1 / pow(tau.clusters, -2) # year heterogeneity in lambda
    tau.year ~ T(dgamma(1, 1), 0.01, 100)
    sd.year <-
      1 / pow(tau.year, -2) # cluster heterogeneity in lambda
    ...
    for (i in 1:nvisits) {
      ...
      loglambda[i] <-
        logDensity[i] + beta[1] * State_P_500[i] + beta[2] * State_P_100[i] + offset[i] +
