...
#    `functionName` is a function's name in your module (e.g., `bayesianBirdModelEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/tmichele/GitHub/borealBirdsAndForestry/modules/bayesianBirdModel/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("bayesianBirdModel")
  ...
  parameters <- list(
    ...
    bayesianBirdModel = list(.saveInitialTime = NA)
  )
  ...
  if (exists("bayesianBirdModelEvent1", envir = .GlobalEnv)) {
    simOutput <- bayesianBirdModelEvent1(mySim)
  } else {
  ...
  if (exists("bayesianBirdModelEvent2", envir = .GlobalEnv)) {
    simOutput <- bayesianBirdModelEvent2(mySim)
  } else {
