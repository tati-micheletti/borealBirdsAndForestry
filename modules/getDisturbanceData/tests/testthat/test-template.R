...
#    `functionName` is a function's name in your module (e.g., `getDisturbanceDataEvent1`).
# 2. Copy this file to the tests folder (i.e., `/home/tmichele/Documents/GitHub/borealBirdsAndForestry/modules/getDisturbanceData/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("getDisturbanceData")
  ...
  parameters <- list(
    ...
    getDisturbanceData = list(.saveInitialTime = NA)
  )
  ...
  if (exists("getDisturbanceDataEvent1", envir = .GlobalEnv)) {
    simOutput <- getDisturbanceDataEvent1(mySim)
  } else {
  ...
  if (exists("getDisturbanceDataEvent2", envir = .GlobalEnv)) {
    simOutput <- getDisturbanceDataEvent2(mySim)
  } else {
